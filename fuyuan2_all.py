...
label_file = "/home/dataset1/zxk_data/Test_data/3Dircadb/label"
image_nii_file = "/home/dataset1/zxk_data/Test_data/3Dircadb/image"
# image_file = "../Dataset/Point_Data/allpoints/all"

image_file = "./save_result/all_yuanwei"     # 这个文件是？
# image_file = "./new_testpoints/AllPoints"     # 这个文件是？
# nii_file ="./nii_image"
Txt_floder = os.listdir(Txt_file)
print("Txt_floder:",Txt_floder)
liver_path = "/home/dataset1/zxk_data/Test_data/3Dircadb/liver"
...
for file_name in Txt_floder:
    ...
    save_point_path = os.path.join(mask_file, mask_name)     # 用于保存最后结果的路径 
    ...
    # print("point_arr[:3, :], image_arr[:3, :]:", point_arr[:10, :])
    image_arr = point_arr
    ...
    # print("-----image_arr.shape, point_arr.shape:", image_arr.shape, point_arr.shape)
    """下面如果是图像坐标,则不需要进行再次转换了:"""
    # image_arr[:, 0] = (image_arr[:, 0] - origin_arr[0]) / (Direction_arr[0, 0] * spacing_arr[0])
    # image_arr[:, 1] = (image_arr[:, 1] - origin_arr[1]) / (Direction_arr[1, 1] * spacing_arr[1])
    # image_arr[:, 2] = (image_arr[:, 2] - origin_arr[2]) / (Direction_arr[2, 2] * spacing_arr[2])
